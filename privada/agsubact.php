...
if (isset($_POST['agsubact'])) {
  ...
  $sql = "INSERT INTO subacts (id, id_act_pert	, actividad, descipcion, invitados)
            VALUES (NULL, '$id_act', '$actividad', '$descripcion', '$invitados')";

  $result = mysqli_query($conn, $sql);

  if ($result) {
    header("Location: panel.php?msg=Registro subact creado exitosamente");
  } else {
      ...
      <form action="" method="post" class="form" enctype="multipart/form-data">
        ...
        <div class="form text-start">
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control border border-dark border-opacity-50" id="actividad" name="actividad" rows="2"
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control border border-dark border-opacity-50" id="descipcion" name="descipcion"
              rows="2" required placeholder="Descripción"></textarea>
          </div>

          <br>
          <div class="mb-3">
            <label class="form-label">Invitar: </label>
            <textarea class="form-control border border-dark border-opacity-50" id="invitados" name="invitados" rows="2"
              placeholder="Correo delimitados por comas"></textarea>
          </div>
<br>
        </div>
        <div class="row">
        ...
        <div class="col-12 col-md-6">
          <button class="btn btn-success" type="submit" name="agsubact">Agregar</button>
        </div>
      </div>
    </div>
    </div>
